...
from os.path import *
import math
FUNC_PATH = f'./{dirname(sys.argv[0])}/'
#print(FUNC_PATH + 'src')
sys.path.insert(1, FUNC_PATH + 'src')
import WordFinder
import AStar
import Grid
...
def run(args, argv):
    ...
    while index < args:
        ...
        if tag == '-r' or tag == '--row':               # SET ROW LENGTH
            ...
            try:
                ...
                print(  # ERROR: param not integer
                    f'Error setting constant: \'{argv[index]}\' not integer.')
                return False
            ...
            if index == args:
                ...
                return False
            try:
                mult = float(argv[index])
            except ValueError:
                print(                                      # ERROR: param not integer
                    f'Error setting mult: \'{argv[index]}\' not integer.')
                return False
    ...
    if not isabs(dictname):
        dictname = FUNC_PATH + 'dictionaries/' + dictname
    try:
...
def export(filepath, grid):
    if not isabs(filepath):
        filepath = FUNC_PATH + 'out/' + filepath
    try:
...
def exportToHTML(fp, grid):
    ...
    for rows in range(grid.rows):
        for cols in range(grid.cols):
            if grid[rows, cols] == Grid.CELL_WALL:
